 ...
 *   reset(snippetName)         — clear panel, show function name in header,
...
function _makeRevealedLine(rawLine, language, lineIdx) {
  ...
  // white-space: pre preserves indentation (set via CSS too, but explicit here)
...
/* Stagger pulse so bars don't all pulse in sync */
...
/* These ensure keywords, strings, and operators are visually distinct */
 ...
 *
 * @param {string}   snippetName — e.g. "reverseString"
...
export function showFull(lines, language) {
  ...
  // Reveal all lines starting from wherever we left off
  for (let idx = _nextRevealIdx; idx < _placeholderEls.length; idx++) {
    const placeholder = _placeholderEls[idx];
    const revealed = _makeRevealedLine(lines[idx] ?? '', language, idx);

    // Stagger the reveal animation so lines cascade in
    const delay = (idx - _nextRevealIdx) * 40; // ms between lines
  ...
  // Scroll to bottom after all lines are revealed
  setTimeout(
    () => {
      _linesContainer.scrollTop = _linesContainer.scrollHeight;
    },
    (_placeholderEls.length - _nextRevealIdx + 1) * 40 + 100
  );
}
 ...
 * @param {string} highlightedLineHTML
 */
